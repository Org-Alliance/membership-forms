...
type CreativesPositionOption = "Chief Creatives Officer" | "Vice Chief Creatives Officer";

const CREATIVES_POSITION_OPTIONS: readonly CreativesPositionOption[] = [
...
export default function CreativesDepartmentPage() {
  ...
  const [selectedPosition, setSelectedPosition] = useState<CreativesPositionOption | "">("");
      ...
      <main className="mx-auto w-full max-w-3xl rounded-2xl border border-sky-100 bg-white/95 p-6 shadow-lg shadow-blue-100 sm:p-8">
        ...
        >
          <fieldset className="space-y-3 rounded-xl border border-sky-200 bg-sky-50/70 p-4 sm:col-span-2">
            ...
            {CREATIVES_POSITION_OPTIONS.map((position) => (
              <label key={position} className="flex items-start gap-3 text-sm">
                <input
                  type="radio"
                  name="creativesPosition"
                  value={position}
                  className="mt-1"
