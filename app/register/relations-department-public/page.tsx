...
type RelationsPositionOption = "Chief Relations Officer" | "Vice Chief Relations Officer";

const RELATIONS_POSITION_OPTIONS: readonly RelationsPositionOption[] = [
...
export default function RelationsDepartmentPage() {
  ...
  const [selectedPosition, setSelectedPosition] = useState<RelationsPositionOption | "">("");
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-sky-50 via-blue-50 to-indigo-100 px-4 py-6 font-sans text-zinc-900">
      <main className="mx-auto w-full max-w-3xl rounded-2xl border border-sky-100 bg-white/95 p-6 shadow-lg shadow-blue-100 sm:p-8">
        ...
        <p className="mt-4 text-sm leading-6 text-slate-700">
          The Relations Department is dedicated to building and strengthening mutually beneficial
          collaborations that align with the organization&apos;s goals. This department identifies and nurtures
          strategic partnerships, facilitates communication with external organizations, and ensures the
          successful execution of joint initiatives.
        </p>
        ...
        >
          <fieldset className="space-y-3 rounded-xl border border-sky-200 bg-sky-50/70 p-4 sm:col-span-2">
            ...
            {RELATIONS_POSITION_OPTIONS.map((position) => (
              <label key={position} className="flex items-start gap-3 text-sm">
                <input
                  type="radio"
                  name="relationsPosition"
                  value={position}
                  className="mt-1"
